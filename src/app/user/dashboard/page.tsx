    ...
    >
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={handleShare} className="mt-4 grid gap-3 md:grid-cols-[1.2fr_1fr_auto]">
          <select className="rounded-2xl border border-slate-200 px-4 py-2 text-sm" value={shareForm.fileId} onChange={(e) => setShareForm({ ...shareForm, fileId: e.target.value })}>
            <option value="">Select file</option>
            {owned.map((file) => (
              <option key={file.id} value={file.id}>{file.filename}</option>
            ))}
          </select>
          <input className="rounded-2xl border border-slate-200 px-4 py-2 text-sm" placeholder="Recipient email" value={shareForm.recipientEmail} onChange={(e) => setShareForm({ ...shareForm, recipientEmail: e.target.value })} />
            ...
            <tbody>
              {owned.map((file) => (
                <tr key={file.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 text-slate-500">Roles: {file.policy?.roles?.join(", ")} • MinC: {file.policy?.minClearance}</td>
